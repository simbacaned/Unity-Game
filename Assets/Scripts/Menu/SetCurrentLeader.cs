...
public class SetCurrentLeader : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    private void Start()
    {
        ...
        if (name == "Nanabozho" && PlayerPrefs.GetInt("leader") == 6)
        {
            myLeaderButtons[6].GetComponent<Image>().color = new Color(0.68f, 1.00f, 0.62f, 0.70f);
        }
        if (name == "Rishi Kashyapa" && PlayerPrefs.GetInt("leader") == 7)
        {
            myLeaderButtons[7].GetComponent<Image>().color = new Color(0.68f, 1.00f, 0.62f, 0.70f);
        }
        if (name == "Zarqa' Al-Yamama" && PlayerPrefs.GetInt("leader") == 8)
        {
            myLeaderButtons[8].GetComponent<Image>().color = new Color(0.68f, 1.00f, 0.62f, 0.70f);
        }
    }
